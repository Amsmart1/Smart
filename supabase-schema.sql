...
-- Enterprise-grade public helpers with mandatory status check (reverted extra reset status checks to only block on active/flagged)
-- Returns NULL if user is inactive (active = FALSE) or flagged (flagged = TRUE) to ensure unblocked login/reset flows.
...
BEGIN
  ...
  IF v_email_raw IS NOT NULL THEN
    ...
    -- Reverted extra reset status check, so we only block on active = FALSE or flagged = TRUE
...
CREATE INDEX IF NOT EXISTS idx_violations_teacher_timestamp ON violations(teacher_email, timestamp DESC);
...
BEGIN
    ...
    LOOP
        ...
        -- Check manual_scores in p_analytics first
        v_manual_pts := NULL;
        IF p_analytics IS NOT NULL AND jsonb_typeof(p_analytics) = 'object' AND p_analytics->'manual_scores' IS NOT NULL AND jsonb_typeof(p_analytics->'manual_scores') = 'object' THEN
            -- Try question id first
            IF v_q->>'id' IS NOT NULL AND p_analytics->'manual_scores' ? (v_q->>'id') THEN
                v_manual_pts := (p_analytics->'manual_scores'->>(v_q->>'id'))::INTEGER;
            -- Try index fallback
            ELSIF p_analytics->'manual_scores' ? (v_idx::TEXT) THEN
                v_manual_pts := (p_analytics->'manual_scores'->>(v_idx::TEXT))::INTEGER;
            END IF;
        END IF;

        IF v_manual_pts IS NOT NULL THEN
            v_score := v_score + v_manual_pts;
        ELSE
...
BEGIN
    FOR v_sub IN
        SELECT qs.id, qs.quiz_id, qs.answers, qs.analytics, qs.started_at,
               LEAST(
...
BEGIN
    ...
    -- Enterprise Grade Optimization: Batch deletions using CTE with LIMIT 500
    WITH to_delete AS (SELECT id FROM broadcasts WHERE expires_at < NOW() LIMIT 500)
    DELETE FROM broadcasts WHERE id IN (SELECT id FROM to_delete);

    -- Hard limit: Delete all notifications older than 90 days
    WITH to_delete AS (SELECT id FROM notifications WHERE created_at < v_expired_cutoff LIMIT 500)
    DELETE FROM notifications WHERE id IN (SELECT id FROM to_delete);

    -- Soft limit: Delete read notifications older than 30 days
    WITH to_delete AS (SELECT id FROM notifications WHERE created_at < (NOW() - INTERVAL '30 days') AND is_read = TRUE LIMIT 500)
    DELETE FROM notifications WHERE id IN (SELECT id FROM to_delete);

    WITH to_delete AS (SELECT id FROM violations WHERE expires_at < NOW() LIMIT 500)
    DELETE FROM violations WHERE id IN (SELECT id FROM to_delete);
