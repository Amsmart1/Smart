...
class AIManager {
    ...
    static renderChatbot(containerId, options = {}) {
        ...
        const appendMessage = (role, content) => {
            ...
            // Enterprise Grade Safety: Escape untrusted AI content before applying markdown rendering
            const escaped = window.escapeHtml(content);

            // Format content (safe markdown-like replacement on escaped string)
            // 1. Links: [text](url) - Only allow http/https protocols via window.isValidUrl
            // 2. Bullet points: * item or - item (at start of line)
            // 3. Bold: **text**
            // 4. Newlines to <br>
            const formatted = escaped
                .replace(/\[(.*?)\]\((.*?)\)/g, (match, text, url) => {
                    const decodedUrl = url.replace(/&amp;/g, '&'); // Fix escaped ampersands in URL
                    if (window.isValidUrl(decodedUrl)) {
                        return `<a href="${window.escapeAttr(decodedUrl)}" target="_blank" class="text-link" rel="noopener noreferrer">${text}</a>`;
                    }
                    return match;
                })
                .replace(/^\s*[\*\-]\s+(.*)$/gm, '• $1')
