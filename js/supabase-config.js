...
class SupabaseDB {
    ...
    static async getViolations(assessmentId = null, userEmail = null, teacherEmail = null, options = {}) {
        const { assessmentType = null, severity = null, sessionId = null, attemptId = null } = options;
        return this._request(async () => {
            let query = supabaseClient.from('violations').select('*', { count: 'exact' });

            if (attemptId || sessionId) query = query.eq('attempt_id', attemptId || sessionId);
     ...
     */
    static async getViolationMedia(attemptId) {
        return this._request(async () => {
            const { data, error } = await supabaseClient
                .from('violations')
                .select('*')
