...
class SupabaseDB {
    ...
    static TABLE_WHITELISTS = {
        users: ['email', 'full_name', 'phone', 'role', 'active', 'notification_preferences', 'metadata', 'updated_at'],
    ...
    static _sanitizePayload(payload, table = null) {
        ...
        VIRTUAL_FIELDS.forEach(field => {
            ...
            if (table === 'users' && (field === 'full_name' || field === 'role')) return;
            if (table === 'invites' && field === 'role') return;
