...
async function terminateSession(attemptId, email) {
    ...
    try {
        ...
        // We omit assessment metadata to let the DB trigger inherit it from the effort history, ensuring correct course/teacher links.
...
async function sendMessageToStudent(email, attemptId = null) {
    ...
    try {
        ...
        // We omit assessment metadata to let the DB trigger inherit it from the effort history.
        if (attemptId) {
            await SupabaseDB.saveViolation({
                attempt_id: attemptId,
                user_email: email,
