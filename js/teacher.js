...
window.applyAIGradingFeedback = function(overallFeedback, questionsJsonStr) {
    try {
        const questions = JSON.parse(decodeURIComponent(questionsJsonStr));
        ...
        if (isStructured) {
            modalBody = `
                <div class="p-10">
                    ...
                    <div class="small" style="line-height: 1.6; max-height: 400px; overflow-y: auto; padding: 15px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">
                    ...
                    <div class="flex gap-10 mt-15">
                        <button class="button success small w-auto" onclick="window.applyAIGradingFeedback(\`${overallFeedbackText.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`, '${encodeURIComponent(JSON.stringify(questionData))}'); this.closest('.modal-backdrop').remove();">Apply Feedback</button>
            ...
            modalBody = `
                <div class="p-10">
                    ...
                    <p class="tiny text-muted italic">Note: These insights are generated by AI and should be verified by the teacher before finalizing the grade.</p>
                    <div class="flex gap-10 mt-15">
                        <button class="button success small w-auto" onclick="const fb=document.getElementById('feedback'); if(fb) fb.value += (fb.value ? '\\n\\n' : '') + 'AI Insight:\\n' + \`${reportText.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`; this.closest('.modal-backdrop').remove();">Apply Feedback</button>
                        <button class="button secondary small w-auto" onclick="this.closest('.modal-backdrop').remove();">Cancel</button>
                    </div>
                </div>
            `;
